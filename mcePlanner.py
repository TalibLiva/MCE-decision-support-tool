...
def find_nearest_crane_company(turbine_lat, turbine_lon, crane_df, required_capacity=None):
    ...
    if required_capacity:
        available_cranes = crane_df[crane_df["Min Capacity (t)"] >= required_capacity].copy()
    else:
...
if pd.notna(turbine_data['latitude']) and pd.notna(turbine_data['longitude']):
    ...
    if nearest_cranes is not None and len(nearest_cranes) > 0:
        ...
        for idx, crane in top_cranes.iterrows():
            with st.expander(f"🏆 #{top_cranes.index.get_loc(idx) + 1}: {crane['Company Name']} - {crane['distance_km']:.2f} km away"):
                col1, col2, col3 = st.columns(3)
                
                with col1:
                    st.write("**Crane Class:**", crane['Crane Class'])
                    st.write("**Min Capacity:**", f"{crane['Min Capacity (t)']} tonnes")
                
                with col2:
                    st.write("**Mobilization:**", f"{crane['Mobilization Days']} days")
                    st.write("**Daily Cost:**", f"€{crane['Daily Cost (€)']:,}")
                
                with col3:
                    st.write("**Max Wind:**", f"{crane['Max Wind (m/s)']} m/s")
                    st.write("**Distance:**", f"{crane['distance_km']:.2f} km")
                
                if pd.notna(crane['Notes']) and crane['Notes'] != '':
                    st.write("**Notes:**", crane['Notes'])
